...
tf.flags.DEFINE_string("positive_data_file", "pos_labeled_data_set.csv", "Data source for the positive data.")
tf.flags.DEFINE_string("negative_data_file", "neg_labeled_data_set.csv", "Data source for the negative data.")
tf.flags.DEFINE_string("neutral_data_file", "neu_labeled_data_set.csv", "Data source for the neutral data.")
tf.flags.DEFINE_integer("num_labels", 3, "Number of labels for data. (default: 2)")

# Model hyperparameters
# filter_sizes - 这个参数确定我们希望我们的卷积核每次覆盖几个单词。
# 对于每个卷积核，我们都将有 num_filters 个。比如，filter_sizes = [3, 4, 5] ,
# 这就意味着，卷积核一共有三种类型，分别是每次覆盖3个单词的卷积核，
# 每次覆盖4个单词的卷积核和每次覆盖5个单词的卷积核。卷积核一共的数量是 3 * num_filters 个。
...
# Training paramters
# 一个迭代 = 一个正向通过 + 一个反向通过 ； 一个时期（epoch） = 所有训练样本的一个正向传递和一个反向传递 ；
# batchsize 的正确选择是为了在内存效率和内存容量之间寻找最佳平衡。
# epoch 保证随机抽样的每个样本都被抽到
# evaluate_every 每训练100次测试一下
tf.flags.DEFINE_integer("batch_size", 64, "Batch Size (default: 64)") # Creates batches by randomly shuffling tensors.
...
# Load data
print("Loading data...")
x_text, y = data_helpers.load_positive_negative_data_files(FLAGS.positive_data_file, FLAGS.negative_data_file,FLAGS.neutral_data_file)
